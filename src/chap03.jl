...
# ╔═╡ e5166721-bb74-4134-a3ee-a7a3e844fe27
begin
	using Flux
	using Plots
	using PlutoUI
...
# ╔═╡ 94faed58-97b5-11ec-3c1c-fb4c82bfe722
md"""
# Chapter 3
#### Kazuharu Yanagimoto
...
# ╔═╡ ead4e096-a944-4890-9fa1-9f13405b4b55
...
# ╔═╡ 3e61cd68-cf44-4018-83c2-9970937cfd86
function create_contexts_target(corpus, n_window = 1)
	target = corpus[begin+n_window:end-n_window]

	contexts = Vector{Vector{Int}}()
	for i = 1+n_window:length(corpus)-n_window
		cs = Vector{Int}()
		for t = -n_window:n_window
			if t != 0
				push!(cs, corpus[i + t])
			end
		end
...
# ╔═╡ 1116b512-7927-4fc4-a52a-e6046fc46603
begin
	text = "You say goodbe and I say hello."
	corpus, word_to_id, id_to_word = preprocess(text)
	n_vocab = length(word_to_id)
	
	contexts, target = create_contexts_target(corpus)
	y = Flux.onehotbatch(target, 1:n_vocab)
	x₁ = hcat([Flux.onehot(cs[1], 1:n_vocab) for cs ∈ contexts]...)
	x₂ = hcat([Flux.onehot(cs[2], 1:n_vocab) for cs ∈ contexts]...)
end

# ╔═╡ 4f510ada-5606-41ba-9c43-ecba20400e4c
md"""
### CBOW model
"""

# ╔═╡ 9a467642-07df-4979-a3f1-9269e7d1ba44
begin
	# Define a Share Layer for the input
	struct Share{T}
		W::T
	end

	Share(in₁::Integer, in₂::Integer, out::Integer) = Share(randn(out, in₁))
	(m::Share)((x₁, x₂)) = m.W * 0.5x₁ .+ m.W * 0.5x₂
end

# ╔═╡ e0f0a8d1-ea71-4664-a92e-35915d0757b0
begin
	n_hidden = 5
	n_batch = 3
	n_epoch = 1000
	...
	model = Chain(
		Share(n_vocab, n_vocab, n_hidden),
		Dense(n_hidden, n_vocab),
		softmax
	)

	loss(x₁, x₂, y) = Flux.crossentropy(model((x₁, x₂)), y)
	train_batched = Flux.DataLoader((x₁, x₂, y), batchsize=n_batch, shuffle=true)

	# Training
	losses = Vector{Float64}()
	Flux.@epochs n_epoch begin
		Flux.train!(loss, Flux.params(model), train_batched, Flux.ADAM())
		push!(losses, loss(x₁, x₂, y))
	end
end

# ╔═╡ 8a77fbc6-9b41-40ed-8a8e-535edabcbe28
plot(losses, legend = false, xlab = "iterations", ylab = "loss")

# ╔═╡ db614ad3-9830-426c-b06f-e234dad1099c
with_terminal() do
	word_vecs = params(model)[1]
	for (id_word, word) ∈ id_to_word
		println("$word: $(word_vecs[id_word, :])")
	end
end
