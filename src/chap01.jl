...
# ╔═╡ 08d9f386-b4b6-439c-83cf-c04bbb0923b1
begin
	using Plots
	using Flux
	using Random
...
# ╔═╡ d327e082-9721-11ec-3f4f-61e3db24d09c
md"""
# Chapter 1
#### Kazuharu Yanagimoto
...
# ╔═╡ c9571895-a9a6-48da-b532-6b39f102bacd
function load_data(seed=1984)
	Random.seed!(seed)
	n = 100 # Num. sample for each class
	dim = 2 # Data Characteristics
	num_class = 3

	x = zeros(dim, n*num_class)
	t = zeros(Int, num_class, n*num_class)

	for c = 1:num_class, i = 1:n
		r = i / n
		θ = 4c + 4r + 0.2 * randn()

		ix = n * (c-1) + i
		x[:, ix] = [r * sin(θ), r * cos(θ)]
		t[c, ix] = 1
	end
...
# ╔═╡ c0104b64-358d-448f-ae8c-e2f6ad0a89b4
x, t = load_data()

# ╔═╡ c87130a3-d504-4498-8fa7-fe6d5c252b0a
let
	plot()
	n = 100
	for (i, m) in enumerate([:circle, :xcross, :utriangle])
		scatter!(x[1, (i-1)*n+1:i*n], x[2, (i-1)*n+1:i*n], legend = false, shape = m)
	end
	current()
end

# ╔═╡ 1c541074-c7aa-4143-9639-b12d6d7c1832
begin
	n_epoch = 300
	n_batch = 30
	n_hidden = 10
	...
	train_batched = Flux.DataLoader((x, t), batchsize=n_batch, shuffle=true)

	# Training
	losses = Vector{Float64}()
	Flux.@epochs n_epoch begin
		Flux.train!(loss, Flux.params(model), train_batched, Flux.Descent(η))
		push!(losses, loss(x, t))
	end
end

# ╔═╡ 067e8b18-1d3d-459a-b1af-9393b3aab54a
plot(losses, legend = false, xlab = "iterations", ylab = "loss")

# ╔═╡ e4a11d01-e7dd-4295-aab9-f158aec88c50
let
	rs = -1:.002:1
	
	classifier(x, y) = argmax(model([x, y]))
	contour(rs, rs, classifier, f=true, leg=false, alpha=0.2, axis=false, ticks=false)
